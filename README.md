...
Десктопное приложение (Wails + Svelte), оформление в эстетике «studio rack hardware».
...
v1|v2|v3|v4|v5\n
```
Целые значения АЦП 0..1023 через `|`, 115200 бод. Кадр отправляется только
когда хотя бы один ползунок сдвинулся на ≥2 единицы (не чаще 50 Гц), плюс
keepalive-кадр каждые 500 мс. Кроме кадров устройство шлёт строки
`STATE:<theme>,<brightness>,<mode>` (в ответ на `GET` и при любом изменении
темы/яркости/режима, в т.ч. жестами на ползунках) и `PONG` в ответ на `PING`.
...
| `MODE:<n>` | Режим лент: 0 позиция, 1 радуга, 2 эквалайзер (ПК шлёт `M:`) |
| `M:<p1>,…,<p5>` | Пиковые уровни 0..1023 для режима эквалайзера |
| `GET` | Запросить `STATE:<theme>,<brightness>,<mode>` |
...
- Автопереподключение при потере связи (USB выдернули / устройство перезагрузилось)
- Тема и яркость в GUI синхронизированы с устройством (`GET`/`STATE`)
...
| Фронтенд десктопа | Svelte 4, TypeScript, Vite 5 (всё через Wails) |
